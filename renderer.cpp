...
IGL_INLINE void Renderer::RotateX(bool up) {
    ...
    if(!scn->meshPicked){
        //scn rotation
        if(up){  scn->MyRotate(Eigen::Vector3d(1,0,0), 20 / 180.0f);}
        else{   scn->MyRotate(Eigen::Vector3d(1,0,0), -20 / 180.0f);}
    }
    else {
        Matrix3d m = scn->data().Tout.rotation().matrix();
        if (up) {
            scn->data().MyRotate(Eigen::Vector3d(1,0,0), 20 / 180.0f);
        } else {
            scn->data().MyRotate(Eigen::Vector3d(1,0,0), -20 / 180.0f);
        }
    }
...
double Renderer::getDistanceFromSphere(){
    Eigen::Vector3d e = getSphere() -( scn->getTip(scn->numOfCylinder-1)+getButtom());// get the tip of the last cylinder
    return e.norm();
...
IGL_INLINE void Renderer::animateIK() {
    using namespace Eigen;
    Vector3d D=getSphere();
    Vector3d e = scn->getTip(scn->numOfCylinder-1)+getButtom();// get the tip of the last cylinder



//    while (d >0.1 && shouldIK){ // TODO change 0.1 to scn->tol
    for(int i = scn->numOfCylinder-1;shouldIK && getDistanceFromSphere()>0.1   && i>-1 ; i--)
    {
...
//            std::cout<<"buttom is:  " <<r.transpose()<<std::endl;
        ...
        if(alpha<-1)
            alpha=-1;
        Vector3d axis=scn->getParentsRotationMatrixes(i).inverse()*er.cross(rd);
...
IGL_INLINE void Renderer::printAngle() {
    ...
    if(!scn->meshPicked) {
        cout << "the scn  mat is\n "
             << (scn->MakeTransd()).block<3, 3>(0,0) <<endl;

     }
    else{
        if(scn->selected_data_index == 4){return;}
        cout<< "the matrix  of link num \n"<< scn->selected_data_index<<" is \n "<< (scn->getParentsTrans(scn->selected_data_index)*scn->data().MakeTransd().cast<float>()).block<3, 3>(0, 0) << endl;
    }
